...
// ── ULP Enum normalizer ──────────────────────────────────────
// Supabase kolom ulp bertipe ulp_enum dengan nilai 'ULP SARMI', 'ULP ABEPURA', dst.
// index.html kadang kirim 'SARMI' atau 'ULP SARMI' — normalkan ke 'ULP XXXX'.
function _normalizeUlpEnum(ulp) {
  if (!ulp) return null;
  var s = String(ulp).trim().toUpperCase();
  if (!s) return null;
  // Sudah prefixed → kembalikan apa adanya
  if (s.startsWith('ULP ')) return s;
  // Tanpa prefix → tambahkan
  return 'ULP ' + s;
}
...
async function _getDetailLengkap(p, signal) {
  ...
  var g = garduArr[0];
  // Ambil 5 inspeksi terakhir saja (terbaru di atas)
...
async function _getRekap(p, signal) {
  ...
  // Fallback manual — teruskan ulp yang sudah dinormalisasi
  return _getRekapManual({ ulp: ulpFilter }, signal);
}

async function _getRekapManual(p, signal) {
  // ulp sudah dinormalisasi oleh _getRekap, tapi normalkan lagi untuk keamanan
  var ulpFilter = (p && p.ulp) ? _normalizeUlpEnum(p.ulp) : null;
  var now = new Date();
...
async function _getGarduKritis(p, signal) {
  // Normalisasi ULP ke format enum Supabase: 'SARMI' → 'ULP SARMI'
  var ulpFilter = (p && p.ulp) ? _normalizeUlpEnum(p.ulp) : null;
